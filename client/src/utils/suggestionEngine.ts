...
export function extractContextFromWorkOrder(
  workOrder: any,
  executionData: any
): WorkOrderContext {
  return {
    ...
    componentCategory:
      workOrder?.component_category || workOrder?.componentCategory,
    maintenanceType:
      workOrder?.maintenance_basis || workOrder?.maintenanceBasis,
    frequency:
      workOrder?.frequency_value?.toString() ||
      workOrder?.frequencyValue?.toString(),

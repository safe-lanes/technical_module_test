...
export default function ModifyPMS() {
  ...
  const [editingRequest, setEditingRequest] = useState<ChangeRequest | null>(
    null
  );
  ...
  useEffect(() => {
    const returnWithTarget = sessionStorage.getItem(
    ...
    if (returnWithTarget === 'true' && formDataStr) {
      ...
      if (editingId && requests) {
        const requestToEdit = requests.find(
          (r: ChangeRequest) => r.id === parseInt(editingId)
        );
        if (requestToEdit) {
              ...
              {isLoading ? (
                ...
                <div className='h-full flex flex-col'>
                  ...
                  <div className='mt-auto px-6 py-4 border-t border-gray-200 text-sm text-gray-500'>
                    {requests.length > 0
                      ? `0 to ${requests.length} of ${requests.length}`
                      : '0 to 0 of 0'}
                  </div>
                </div>
              )}
                    ...
                    <div className='flex items-center gap-2'>
                      ...
                      <div>
                        ...
                        <p className='text-sm text-gray-600'>
                          Target Type:{' '}
                          {formData.targetType?.replace('_', ' ').toUpperCase()}
                        </p>
                      </div>
                    </div>
                    <Button
                      ...
                      onClick={() => {
                        ...
                        setLocation(
                          routes[formData.category as keyof typeof routes] ||
                            `/pms/components?${params}`
                        );
                      }}
                    >
                      Change Target
                ...
                <>
                  <Button
                    ...
                    onClick={() => {
                      ...
                      if (comment) {
                        fetch(
                          `/api/change-requests/${viewingRequest.id}/reject`,
                          {
                            method: 'PUT',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({
                  ...
                  <Button
                    ...
                    onClick={() => {
                      ...
                      if (comment) {
                        fetch(
                          `/api/change-requests/${viewingRequest.id}/approve`,
                          {
                            method: 'PUT',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({

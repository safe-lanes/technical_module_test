...
router.get('/template', function (req, res) {
  ...
  switch (type) {
    case 'components':
      ...
      validValues = __spreadArray(
        ...
        Array.from({ length: 10 }, function () {
          return ['Text', 'Text', 'Number >= 0', 'Yes/No', 'Text'];
        }).flat(),
        true
      );
      example = __spreadArray(
        __spreadArray(
          __spreadArray(
            __spreadArray(
              __spreadArray(
  ...
  res.setHeader(
    'Content-Disposition',
    'attachment; filename="'.concat(type, '_template.xlsx"')
  );
  res.send(buffer);
});
// Dry-run validation
router.post('/dry-run', upload.single('file'), function (req, res) {
      ...
      switch (_b.label) {
        case 0:
          ...
          ((_a = req.body),
            (type = _a.type),
            (mode = _a.mode),
            (archiveMissing = _a.archiveMissing),
      ...
      switch (_d.label) {
        case 0:
          ...
          if (!cachedData) {
            return [
              2 /*return*/,
              res.status(400).json({ error: 'Invalid or expired file token' }),
            ];
          }
          ...
          if (cachedData.results.summary.errors > 0) {
            return [
              2 /*return*/,
              res.status(400).json({ error: 'Cannot import file with errors' }),
            ];
          }
          return [
            4 /*yield*/,
        ...
        case 2:
          ...
          res.json(
            __assign(__assign({}, importResult), { historyId: historyId })
          );
          return [3 /*break*/, 4];
        case 3:
          error_2 = _d.sent();
    ...
    return __generator(this, function (_d) {
      switch (_d.label) {
        case 0:
          ...
          return [
            4 /*yield*/,
            getImportHistory(type, parseInt(limit), parseInt(offset)),
          ];
        case 1:
          history_1 = _d.sent();
          res.json(history_1);
...
router.get('/history/:id/:fileType', function (req, res) {
  return __awaiter(void 0, void 0, void 0, function () {
    ...
    return __generator(this, function (_b) {
      switch (_b.label) {
        ...
        case 1:
          ...
          res.setHeader(
            'Content-Disposition',
            'attachment; filename="'.concat(file.name, '"')
          );
          res.send(file.data);
          return [3 /*break*/, 3];
        case 2:
      ...
      _loop_1 = function (i) {
        ...
        if (type === 'components') {
          ...
          if (!row['Component Category']) {
            errors.push(
              'Row '.concat(rowNum, ': Component Category is required')
            );
          } else if (
            !COMPONENT_CATEGORIES.includes(row['Component Category'])
          ) {
            errors.push(
              'Row '
                .concat(rowNum, ': Invalid Component Category. Allowed: ')
                .concat(COMPONENT_CATEGORIES.join(', '))
            );
          } else {
            normalized['Component Category'] = row['Component Category'];
          }
          // Validate Yes/No fields
          ['Critical (Yes/No)', 'Condition Based (Yes/No)'].forEach(
            function (field) {
              if (row[field]) {
                ...
                if (!['yes', 'no'].includes(value)) {
                  errors.push(
                    'Row '
                      .concat(rowNum, ': ')
                      .concat(field, ' must be Yes or No')
                  );
                } else {
            ...
            if (row[field]) {
              ...
              if (isNaN(num) || num < 0) {
                errors.push(
                  'Row '
                    .concat(rowNum, ': ')
                    .concat(field, ' must be a non-negative number')
                );
              } else {
          ...
          if (row['UOM'] && !UOM_LIST.includes(row['UOM'].toLowerCase())) {
            errors.push(
              'Row '
                .concat(rowNum, ': Invalid UOM. Allowed: ')
                .concat(UOM_LIST.join(', '))
            );
          } else if (row['UOM']) {
          ...
          if (row['Critical (Yes/No)']) {
            ...
            if (!['yes', 'no'].includes(value)) {
              errors.push(
                'Row '.concat(rowNum, ': Critical must be Yes or No')
              );
            } else {
          ...
          if (
            ...
            errors.push(
              'Row '.concat(
                rowNum,
                ': Invalid Type. Allowed: Stores, Lubes, Chemicals, Others'
              )
            );
          } else if (row['Type']) {
  ...
  return __awaiter(this, void 0, void 0, function () {
    ...
    return __generator(this, function (_a) {
      ...
      return [
        2 /*return*/,
        {
          items: filtered.slice(offset, offset + limit).map(function (h) {
            return {
              id: h.id,
              date: h.startedAt,
              user: h.userId,
...
function getHistoryFile(id, fileType) {
  return __awaiter(this, void 0, void 0, function () {
    var history;
    return __generator(this, function (_a) {
      history = (global.importHistory || []).find(function (h) {
        return h.id === id;
      });
      if (!history) return [2 /*return*/, null];
      if (fileType === 'file') {
        return [
          2 /*return*/,
          {

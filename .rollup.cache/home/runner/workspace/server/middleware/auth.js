...
export var authenticateUser = function (username, password) { return __awaiter(void 0, void 0, void 0, function () {
    var user, token;
    return __generator(this, function (_a) {
        ...
        return [2 /*return*/, {
                user: __assign(__assign({}, user), { token: token, refreshToken: generateToken({ userId: user.id, username: user.username, role: user.role, permissions: [], vesselId: user.vesselId }) }),
                token: token
            }];
    });
}); };
...
export var requireRole = function () {
    ...
    return function (req, res, next) {
        ...
        if (!allowedRoles.includes(req.user.role)) {
            return next(new AuthorizationError("Role ".concat(req.user.role, " not authorized")));
        }
        next();
    };
};

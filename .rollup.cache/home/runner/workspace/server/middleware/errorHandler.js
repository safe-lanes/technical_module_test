...
export var globalErrorHandler = function (err, req, res, next) {
    ...
    if (error instanceof ZodError) {
        var validationErrors = error.errors.map(function (err) { return ({
            field: err.path.join('.'),
            message: err.message,
            code: err.code,
        }); });
    ...
    if (error.name === 'ER_NO_REFERENCED_ROW' ||
    ...
    var errorResponse = {
        success: false,
        error: __assign(__assign({ message: appError.message, code: appError.code, status: statusCode }, (isDevelopment && { stack: appError.stack })), (appError.details && { details: appError.details })),
    };

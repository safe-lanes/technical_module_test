...
var ModifyPMS = function () {
    ...
    var _b = useState(""), searchStatus = _b[0], setSearchStatus = _b[1];
    var _c = useState([]), requests = _c[0], setRequests = _c[1];
    var _d = useState(null), selectedRequest = _d[0], setSelectedRequest = _d[1];
    var _e = useState(false), isReviewMode = _e[0], setIsReviewMode = _e[1];
    var _f = useState(""), approvalComment = _f[0], setApprovalComment = _f[1];
    var _g = useState(false), showFormModal = _g[0], setShowFormModal = _g[1];
    var _h = useState(null), formCategory = _h[0], setFormCategory = _h[1];
    var _j = useChangeRequest(), currentUser = _j.currentUser, enterChangeRequestMode = _j.enterChangeRequestMode, exitChangeRequestMode = _j.exitChangeRequestMode;
    useEffect(function () {
    ...
    var filteredRequests = requests.filter(function (request) {
        var matchesCategory = !selectedCategory || request.category === selectedCategory.toLowerCase().replace(' ', '-');
        var matchesSearch = !searchStatus || request.status.toLowerCase().includes(searchStatus.toLowerCase());
    ...
    var handleApprove = function () {
        if (!selectedRequest)
            return;
        changeRequestService.approveChangeRequest(selectedRequest.id, currentUser.name, approvalComment);
        setRequests(changeRequestService.getAllChangeRequests());
    ...
    var handleReject = function () {
        ...
        changeRequestService.rejectChangeRequest(selectedRequest.id, currentUser.name, approvalComment);
        setRequests(changeRequestService.getAllChangeRequests());
        setIsReviewMode(false);
        setSelectedRequest(null);
        setApprovalComment("");
    ...
    return (<div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="mb-6">
        ...
        <div className="flex justify-between items-center mb-4">
          <div className="flex-1 max-w-md">
            <Input placeholder="Search Status" value={searchStatus} onChange={function (e) { return setSearchStatus(e.target.value); }} className="w-full"/>
          </div>
          <Button className="bg-[#52baf3] hover:bg-[#4aa3d9] text-white ml-4" onClick={handleNewChangeRequest}>
      ...
      <div className="flex gap-6">
        ...
        <div className="w-64">
          <div className="bg-white rounded-lg shadow-sm">
            ...
            <div className="p-0">
              {categories.map(function (category) { return (<div key={category.id} className={"px-4 py-3 border-b border-gray-200 cursor-pointer hover:bg-gray-50 ".concat(selectedCategory === category.name ? 'bg-blue-50 border-l-4 border-l-[#52baf3]' : '')} onClick={function () { return setSelectedCategory(selectedCategory === category.name ? null : category.name); }}>
        ...
        <div className="flex-1">
          <div className="bg-white rounded-lg shadow-sm">
            ...
            <div className="divide-y divide-gray-200">
              {filteredRequests.map(function (request) { return (<div key={request.id} className="px-6 py-4 hover:bg-gray-50">
                  <div className="grid grid-cols-5 gap-4 items-center text-sm">
                    <div className="text-gray-900 font-medium cursor-pointer hover:text-[#52baf3]" onClick={function () { return handleViewRequest(request); }}>
                    ...
                    <div>
                      <span className={"px-3 py-1 rounded-full text-xs font-medium ".concat(getStatusColor(request.status))}>
                        {request.status}
                      </span>
                    </div>
                    <div className="flex gap-2 justify-end">
                      <Button variant="ghost" size="sm" onClick={function () { return handleViewRequest(request); }} className="h-8 w-8 p-0">
                        <Eye className="h-4 w-4"/>
                      </Button>
                      {currentUser.role === "approver" && request.status === "Pending Approval" && (<>
                          <Button variant="ghost" size="sm" onClick={function () { return handleViewRequest(request); }} className="h-8 w-8 p-0 text-green-600 hover:text-green-700">
                            <Check className="h-4 w-4"/>
                          </Button>
                          <Button variant="ghost" size="sm" onClick={function () { return handleViewRequest(request); }} className="h-8 w-8 p-0 text-red-600 hover:text-red-700">
                            <X className="h-4 w-4"/>
                          </Button>
                        </>)}
                    </div>
      ...
      {/* Review Modal */}
      {isReviewMode && selectedRequest && (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-lg w-[90%] max-w-4xl max-h-[90vh] overflow-auto">
            <div className="flex justify-between items-center p-6 border-b">
              ...
              <Button variant="ghost" size="sm" onClick={function () { return setIsReviewMode(false); }} className="h-8 w-8 p-0">
                <X className="h-4 w-4"/>
              </Button>
            </div>
            ...
            <div className="p-6">
              <div className="grid grid-cols-2 gap-6 mb-6">
                ...
                <div>
                  ...
                  <span className={"px-3 py-1 rounded-full text-xs font-medium ".concat(getStatusColor(selectedRequest.status))}>
              ...
              <div className="mb-6">
                ...
                <div className="bg-gray-50 rounded-lg p-4">
                  {selectedRequest.changedFields.map(function (field) { return (<div key={field} className="mb-3 last:mb-0">
              ...
              {selectedRequest.comments && (<div className="mb-6">
              ...
              {currentUser.role === "approver" && selectedRequest.status === "Pending Approval" && (<div className="mb-6">
                  <label className="text-sm font-medium text-gray-600 mb-2 block">
                    Approval Comments
                  </label>
                  <textarea value={approvalComment} onChange={function (e) { return setApprovalComment(e.target.value); }} placeholder="Add comments for approval/rejection..." className="w-full p-3 border border-gray-300 rounded-lg resize-none" rows={3}/>
                </div>)}

              {selectedRequest.rejectionReason && (<div className="mb-6">
            ...
            <div className="flex gap-3 justify-end p-6 border-t bg-gray-50">
              <Button variant="outline" onClick={function () { return setIsReviewMode(false); }}>
                Close
              </Button>
              {currentUser.role === "approver" && selectedRequest.status === "Pending Approval" && (<>
                  <Button variant="outline" onClick={handleReject} className="text-red-600 border-red-600 hover:bg-red-50">
                    Reject
                  </Button>
      ...
      {/* Form Modal */}
      {showFormModal && (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="flex items-center justify-center w-full h-full p-4">
            {formCategory === "Components" && (<ComponentChangeRequestForm onClose={handleCloseFormModal} onSubmit={function (componentData) {
            ...
            {formCategory === "Work orders" && (<WorkOrdersChangeRequestForm onClose={handleCloseFormModal} onSubmit={function (workOrderData) {
            ...
            {formCategory === "Spares" && (<SparesChangeRequestForm onClose={handleCloseFormModal} onSubmit={function (sparesData) {
            ...
            {formCategory === "Stores" && (<StoresChangeRequestForm onClose={handleCloseFormModal} onSubmit={function (storesData) {

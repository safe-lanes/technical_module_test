...
import { Search, Edit, FileSpreadsheet, X, Calendar, Plus, Archive } from "lucide-react";
...
var Stores = function () {
    ...
    var _a = useState("stores"), activeTab = _a[0], setActiveTab = _a[1];
    var _b = useState("inventory"), viewMode = _b[0], setViewMode = _b[1];
    var _c = useState(""), searchTerm = _c[0], setSearchTerm = _c[1];
    var _d = useState(""), categoryFilter = _d[0], setCategoryFilter = _d[1];
    var _e = useState(""), stockFilter = _e[0], setStockFilter = _e[1];
    var _f = useState(""), vesselFilter = _f[0], setVesselFilter = _f[1];
    var _g = useState(false), isBulkUpdateModalOpen = _g[0], setIsBulkUpdateModalOpen = _g[1];
    var _h = useState({}), bulkUpdateData = _h[0], setBulkUpdateData = _h[1];
    var _j = useState(""), placeReceived = _j[0], setPlaceReceived = _j[1];
    var _k = useState(""), dateReceived = _k[0], setDateReceived = _k[1];
    var _l = useState(storeItems), items = _l[0], setItems = _l[1];
    // History filters
    var _m = useState(""), historyDateFrom = _m[0], setHistoryDateFrom = _m[1];
    var _o = useState(""), historyDateTo = _o[0], setHistoryDateTo = _o[1];
    var _p = useState(""), historySearch = _p[0], setHistorySearch = _p[1];
    var _q = useState("all"), historyEventFilter = _q[0], setHistoryEventFilter = _q[1];
    var _r = useState([
        {
            id: 1001,
    ...
    ]), historyItems = _r[0], setHistoryItems = _r[1];
    // Edit modal state
    var _s = useState(false), isEditModalOpen = _s[0], setIsEditModalOpen = _s[1];
    var _t = useState(null), editingItem = _t[0], setEditingItem = _t[1];
    var _u = useState({
        itemName: "",
        uom: "",
        customUom: "",
        ...
        location: "",
        notes: ""
    }), editForm = _u[0], setEditForm = _u[1];
    // Receive modal state
    var _v = useState(false), isReceiveModalOpen = _v[0], setIsReceiveModalOpen = _v[1];
    var _w = useState(null), receivingItem = _w[0], setReceivingItem = _w[1];
    var _x = useState({
        quantity: "",
    ...
    var filteredItems = useMemo(function () {
        ...
        return updatedItems.filter(function (item) {
            ...
            var matchesCategory = !categoryFilter || categoryFilter === "all" || item.storesCategory.includes(categoryFilter);
            var matchesStock = !stockFilter || stockFilter === "all" || item.stock.toLowerCase() === stockFilter.toLowerCase();
    ...
    var exportInventoryToExcel = function () {
        ...
        toast({ title: "Export Successful", description: "Exported ".concat(data.length, " items to ").concat(filename) });
    };
    var exportHistoryToExcel = function () {
        ...
        XLSX.writeFile(wb, filename);
        toast({ title: "Export Successful", description: "Exported ".concat(data.length, " entries to ").concat(filename) });
    };
    ...
    var saveBulkUpdates = function () {
        ...
        var updatedItems = items.map(function (item) {
            ...
            if (!updateData)
                return item;
            var consumed = updateData.consumed || 0;
            var received = updateData.received || 0;
        ...
        toast({
            ...
            description: "Updated: ".concat(updatedCount, ", Skipped: ").concat(skippedCount, ", Failed: ").concat(failedCount),
        });
    };
    // Handle Edit Item
    ...
    var saveEditItem = function () {
        if (!editingItem)
            return;
        var uom = editForm.uom === "Other" ? editForm.customUom : editForm.uom;
        var updatedItems = items.map(function (item) {
    ...
    var saveReceive = function () {
        ...
        var newRob = receivingItem.rob + quantity;
        var updatedItems = items.map(function (item) {
        ...
        toast({ title: "Success", description: "Received ".concat(quantity, " ").concat(receivingItem.uom || 'units') });
    };
    // Handle Archive
    ...
    return (<div className="flex-1 p-6 bg-gray-50 min-h-screen">
      ...
      {/* Table */}
      {viewMode === "inventory" ? (<div className="bg-white rounded-lg shadow overflow-hidden">
        {/* Table Header */}
        ...
        <div className="divide-y divide-gray-200">
          {filteredItems.map(function (item) { return (<div key={item.id} className="p-4 hover:bg-gray-50">
              <div className="grid grid-cols-12 gap-4 items-center text-sm">
                ...
                <div className="col-span-1">
                  <span className={"px-2 py-1 rounded text-xs font-medium ".concat(getStockColor(item.stock))}>
                    {item.stock}
                  </span>
                </div>
                ...
                <div className="col-span-1 flex gap-1">
                  <Button variant="ghost" size="sm" className="h-8 w-8 p-0" onClick={function () { return openEditModal(item); }} title="Edit Item">
                    <Edit className="h-4 w-4 text-gray-500"/>
                  </Button>
                  <Button variant="ghost" size="sm" className="h-8 w-8 p-0" onClick={function () { return openReceiveModal(item); }} title="Receive">
                    <Plus className="h-4 w-4 text-gray-500"/>
                  </Button>
                  <Button variant="ghost" size="sm" className="h-8 w-8 p-0" onClick={function () { return handleArchive(item); }} title="Archive">
          ...
          {filteredHistoryItems.length === 0 ? (<div className="p-8 text-center text-gray-500">
              No history entries found. Actions like Receive, Consume, and Archive will appear here.
                ...
                <div className="grid grid-cols-12 gap-4 items-center text-sm">
                  ...
                  <div className="col-span-1">
                    <span className={"px-2 py-1 rounded text-xs font-medium ".concat(item.eventType === 'RECEIVE' ? 'bg-green-100 text-green-800' :
        ...
        <DialogContent className="sm:max-w-[425px]">
          ...
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              ...
              <Input id="itemName" value={editForm.itemName} onChange={function (e) { return setEditForm(__assign(__assign({}, editForm), { itemName: e.target.value })); }}/>
            </div>
            <div className="grid gap-2">
              <Label htmlFor="uom">Unit of Measure</Label>
              <Select value={editForm.uom} onValueChange={function (value) { return setEditForm(__assign(__assign({}, editForm), { uom: value })); }}>
                <SelectTrigger>
                  <SelectValue placeholder="Select UOM"/>
                </SelectTrigger>
                <SelectContent>
                  {UOM_OPTIONS.map(function (opt) { return (<SelectItem key={opt} value={opt}>{opt}</SelectItem>); })}
                </SelectContent>
              </Select>
              {editForm.uom === "Other" && (<Input placeholder="Enter custom UOM" value={editForm.customUom} onChange={function (e) { return setEditForm(__assign(__assign({}, editForm), { customUom: e.target.value })); }}/>)}
            </div>
            <div className="grid gap-2">
              <Label htmlFor="min">Minimum Stock</Label>
              <Input id="min" type="number" min="0" value={editForm.min} onChange={function (e) { return setEditForm(__assign(__assign({}, editForm), { min: parseInt(e.target.value) || 0 })); }}/>
            </div>
            <div className="grid gap-2">
              <Label htmlFor="location">Location</Label>
              <Input id="location" value={editForm.location} onChange={function (e) { return setEditForm(__assign(__assign({}, editForm), { location: e.target.value })); }}/>
            </div>
            <div className="grid gap-2">
              <Label htmlFor="notes">Notes</Label>
              <Textarea id="notes" value={editForm.notes} onChange={function (e) { return setEditForm(__assign(__assign({}, editForm), { notes: e.target.value })); }} rows={3}/>
            </div>
          </div>
          <DialogFooter>
        ...
        <DialogContent className="sm:max-w-[425px]">
          ...
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              ...
              <Input id="quantity" type="number" min="1" value={receiveForm.quantity} onChange={function (e) { return setReceiveForm(__assign(__assign({}, receiveForm), { quantity: e.target.value })); }}/>
            </div>
            <div className="grid gap-2">
              <Label htmlFor="dateLocal">Date Received</Label>
              <Input id="dateLocal" type="date" value={receiveForm.dateLocal} onChange={function (e) { return setReceiveForm(__assign(__assign({}, receiveForm), { dateLocal: e.target.value })); }}/>
            </div>
            <div className="grid gap-2">
              <Label htmlFor="place">Place/Port</Label>
              <Input id="place" value={receiveForm.place} onChange={function (e) { return setReceiveForm(__assign(__assign({}, receiveForm), { place: e.target.value })); }}/>
            </div>
            <div className="grid gap-2">
              <Label htmlFor="supplierPO">Supplier/PO#</Label>
              <Input id="supplierPO" value={receiveForm.supplierPO} onChange={function (e) { return setReceiveForm(__assign(__assign({}, receiveForm), { supplierPO: e.target.value })); }}/>
            </div>
            <div className="grid gap-2">
              <Label htmlFor="remarks">Remarks</Label>
              <Textarea id="remarks" value={receiveForm.remarks} onChange={function (e) { return setReceiveForm(__assign(__assign({}, receiveForm), { remarks: e.target.value })); }} rows={3}/>
            </div>
          </div>
          <DialogFooter>
          ...
          <div className="bg-white rounded-lg shadow-lg w-[95%] max-w-7xl max-h-[90vh] overflow-auto">
            ...
            <div className="p-6">
              ...
              <div className="border border-t-0 rounded-b max-h-[400px] overflow-y-auto">
                ...
                var newRob = item.rob - consumed + received;
                var hasError = newRob < 0 || (received > 0 && !((_c = bulkUpdateData[item.id]) === null || _c === void 0 ? void 0 : _c.receivedDate));
                return (<div key={item.id} className={"grid grid-cols-9 gap-3 p-3 border-b ".concat(hasError ? 'bg-red-50' : 'bg-white', " items-center")}>
                      ...
                      <div>
                        <Input type="number" min="0" max={item.rob} className={"text-sm h-8 ".concat(newRob < 0 ? 'border-red-500' : '')} placeholder="0" value={consumed || ''} onChange={function (e) { return handleBulkUpdateChange(item.id, 'consumed', e.target.value); }}/>
                      </div>
                      <div>
                        <Input type="number" min="0" className="text-sm h-8" placeholder="0" value={received || ''} onChange={function (e) { return handleBulkUpdateChange(item.id, 'received', e.target.value); }}/>
                      </div>
                      ...
                      <div>
                        <Input type="text" className="text-sm h-8" placeholder="Comments" value={((_d = bulkUpdateData[item.id]) === null || _d === void 0 ? void 0 : _d.comments) || ''} onChange={function (e) { return handleBulkUpdateChange(item.id, 'comments', e.target.value); }}/>
                      </div>
                    </div>);
            })}
              </div>

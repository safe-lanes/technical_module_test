...
var Stores = function () {
  ...
  var _a = useState('stores'),
    activeTab = _a[0],
    setActiveTab = _a[1];
  var _b = useState('inventory'),
    viewMode = _b[0],
    setViewMode = _b[1];
  var _c = useState(''),
    searchTerm = _c[0],
  ...
  var _f = useState(''),
    vesselFilter = _f[0],
    setVesselFilter = _f[1];
  var _g = useState(false),
    isBulkUpdateModalOpen = _g[0],
    setIsBulkUpdateModalOpen = _g[1];
  var _h = useState({}),
    bulkUpdateData = _h[0],
    setBulkUpdateData = _h[1];
  var _j = useState(''),
    placeReceived = _j[0],
    setPlaceReceived = _j[1];
  var _k = useState(''),
    dateReceived = _k[0],
    setDateReceived = _k[1];
  var _l = useState(storeItems),
    items = _l[0],
    setItems = _l[1];
  // History filters
  var _m = useState(''),
    historyDateFrom = _m[0],
    setHistoryDateFrom = _m[1];
  var _o = useState(''),
    historyDateTo = _o[0],
    setHistoryDateTo = _o[1];
  var _p = useState(''),
    historySearch = _p[0],
    setHistorySearch = _p[1];
  var _q = useState('all'),
    historyEventFilter = _q[0],
    setHistoryEventFilter = _q[1];
  var _r = useState([
      {
        id: 1001,
    ...
    historyItems = _r[0],
    setHistoryItems = _r[1];
  // Edit modal state
  var _s = useState(false),
    isEditModalOpen = _s[0],
    setIsEditModalOpen = _s[1];
  ...
  var _v = useState(false),
    isReceiveModalOpen = _v[0],
    setIsReceiveModalOpen = _v[1];
  var _w = useState(null),
    receivingItem = _w[0],
    setReceivingItem = _w[1];
  var _x = useState({
      quantity: '',
  ...
  var addToHistory = function (
    ...
    setHistoryItems(function (prev) {
      return __spreadArray([historyEntry], prev, true);
    });
  };
  ...
  var exportInventoryToExcel = function () {
    ...
    var timestamp = now
      ...
      .replace('T', '_')
      .slice(0, 15);
    var filename = 'stores_'
      .concat(activeTab, '_inventory_')
      .concat(timestamp, '.xlsx');
  ...
  var exportHistoryToExcel = function () {
    ...
    var timestamp = now
      ...
      .slice(0, 15);
    var filename = 'stores_'
      .concat(activeTab, '_history_')
      .concat(timestamp, '.xlsx');
    var data = filteredHistoryItems.map(function (item) {
      return {
        Date: item.dateLocal,
        ...
        'Qty Change':
          item.qtyChange > 0
            ? '+'.concat(item.qtyChange)
            : item.qtyChange.toString(),
    ...
    toast({
      title: 'Export Successful',
      description: 'Exported '
        .concat(data.length, ' entries to ')
        .concat(filename),
    });
  };
  // Filter history items
  var filteredHistoryItems = useMemo(
    function () {
    ...
    if (field === 'consumed' || field === 'received') {
      ...
      setBulkUpdateData(function (prev) {
        ...
        return __assign(
          ...
          ((_a = {}),
          (_a[itemId] = __assign(
            __assign({}, prev[itemId]),
            ((_b = {}), (_b[field] = numValue_1), _b)
          )),
          _a)
        );
      });
  ...
  var saveBulkUpdates = function () {
    ...
    toast({
      ...
      description: 'Updated: '
        .concat(updatedCount, ', Skipped: ')
        .concat(skippedCount, ', Failed: ')
        .concat(failedCount),
    });
  };
  // Handle Edit Item
      ...
      if (item.id === editingItem.id) {
        ...
        if (item.min !== editForm.min) {
          addToHistory(
            ...
            '',
            '',
            'Min changed from '.concat(item.min, ' to ').concat(editForm.min)
          );
        }
  ...
  var saveReceive = function () {
    ...
    toast({
      ...
      description: 'Received '
        .concat(quantity, ' ')
        .concat(receivingItem.uom || 'units'),
    });
  };
  // Handle Archive
  var handleArchive = function (item) {
    var confirmMessage =
      item.rob > 0
        ? 'This item has stock on hand (ROB = '.concat(
            item.rob,
            '). Archive anyway?'
          )
    ...
    if (confirm(confirmMessage)) {
      var updatedItems = items.map(function (i) {
        return i.id === item.id
          ? __assign(__assign({}, i), { isArchived: true })
          : i;
      });
      // Add to history
  ...
  return (
    <div className='flex-1 p-6 bg-gray-50 min-h-screen'>
      ...
      <div className='flex gap-1 mb-6'>
        <button
          onClick={function () {
            return setActiveTab('stores');
          }}
          className={'px-6 py-2 rounded-t text-sm font-medium '.concat(
            activeTab === 'stores'
        ...
        <div className='flex gap-4 mb-6'>
          <div className='flex-1'>
            <Input
              ...
              value={vesselFilter}
              onChange={function (e) {
                return setVesselFilter(e.target.value);
              }}
              className='text-sm'
            />
          </div>
        ...
        <div className='flex gap-4 mb-6'>
          <div className='flex-1 relative'>
            ...
            <Input
              ...
              value={historySearch}
              onChange={function (e) {
                return setHistorySearch(e.target.value);
              }}
              className='pl-10 text-sm'
            />
          </div>
          ...
          <div className='flex items-center gap-2'>
            <Input
              ...
              onChange={function (e) {
                return setHistoryDateFrom(e.target.value);
              }}
              className='text-sm'
              placeholder='From'
            />
            ...
            <Input
              ...
              value={historyDateTo}
              onChange={function (e) {
                return setHistoryDateTo(e.target.value);
              }}
              className='text-sm'
              placeholder='To'
            />
        ...
        <div className='bg-white rounded-lg shadow overflow-hidden'>
          ...
          <div className='divide-y divide-gray-200'>
            {filteredHistoryItems.length === 0 ? (
              ...
              filteredHistoryItems.map(function (item) {
                return (
                  <div key={item.id} className='p-4 hover:bg-gray-50'>
                    <div className='grid grid-cols-12 gap-4 items-center text-sm'>
                      <div className='col-span-2 text-gray-700'>
          ...
          <div className='grid gap-4 py-4'>
            ...
            <div className='grid gap-2'>
              ...
              <Select
                ...
                onValueChange={function (value) {
                  return setEditForm(
                    __assign(__assign({}, editForm), { uom: value })
                  );
                }}
              >
                <SelectTrigger>
            ...
            <div className='grid gap-2'>
              ...
              <Textarea
                ...
                onChange={function (e) {
                  return setEditForm(
                    __assign(__assign({}, editForm), { notes: e.target.value })
                  );
                }}
                rows={3}
              />
      ...
      <Dialog open={isReceiveModalOpen} onOpenChange={setIsReceiveModalOpen}>
        <DialogContent className='sm:max-w-[425px]'>
          <DialogHeader>
            <DialogTitle>
              Receive{' '}
              {receivingItem === null || receivingItem === void 0
                ? void 0
                : receivingItem.itemName}
            </DialogTitle>
          </DialogHeader>
          <div className='grid gap-4 py-4'>
            <div className='grid gap-2'>
              ...
              <Input
                ...
                onChange={function (e) {
                  return setReceiveForm(
                    __assign(__assign({}, receiveForm), {
          ...
          <DialogFooter>
            <Button
              variant='outline'
              onClick={function () {
                return setIsReceiveModalOpen(false);
              }}
            >
              Cancel
            </Button>
            ...
            <div className='p-6'>
              ...
              <div className='border border-t-0 rounded-b max-h-[400px] overflow-y-auto'>
                {filteredItems.map(function (item) {
                  var _a, _b, _c, _d;
                  var consumed =
                    ((_a = bulkUpdateData[item.id]) === null || _a === void 0
                      ? void 0
                      : _a.consumed) || 0;
                  var received =
                    ((_b = bulkUpdateData[item.id]) === null || _b === void 0
                      ? void 0
                      : _b.received) || 0;
                  var newRob = item.rob - consumed + received;
                  var hasError =
                    newRob < 0 ||
                    (received > 0 &&
                      !((_c = bulkUpdateData[item.id]) === null || _c === void 0
                        ? void 0
                        : _c.receivedDate));
                  return (
                    <div
                      key={item.id}
                      className={'grid grid-cols-9 gap-3 p-3 border-b '.concat(
                    ...
                    >
                      ...
                      <div>
                        <Input
                          ...
                          min='0'
                          max={item.rob}
                          className={'text-sm h-8 '.concat(
                            newRob < 0 ? 'border-red-500' : ''
                          )}
                          placeholder='0'
                          value={consumed || ''}
                          onChange={function (e) {
                      ...
                      <div>
                        <Input
                          ...
                          value={
                            ((_d = bulkUpdateData[item.id]) === null ||
                            _d === void 0
                              ? void 0
                              : _d.comments) || ''
                          }

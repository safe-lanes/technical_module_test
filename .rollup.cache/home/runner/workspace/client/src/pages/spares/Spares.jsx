...
import { Search, ChevronRight, ChevronDown, Edit, Trash2, FileSpreadsheet, X, MessageSquare, Calendar, } from 'lucide-react';
import AgGridTable from '@/components/common/AgGridTable';
import { StockStatusCellRenderer, CriticalCellRenderer, SparesActionsCellRenderer, } from '@/components/common/AgGridCellRenderers';
...
var Spares = function () {
    ...
    var _g = useState(false), isAddSpareModalOpen = _g[0], setIsAddSpareModalOpen = _g[1];
    var _h = useState(false), isBulkUpdateModalOpen = _h[0], setIsBulkUpdateModalOpen = _h[1];
    var _j = useState({}), bulkUpdateData = _j[0], setBulkUpdateData = _j[1];
    var _k = useState(''), placeReceived = _k[0], setPlaceReceived = _k[1];
    var _l = useState(''), dateReceived = _l[0], setDateReceived = _l[1];
    ...
    var filteredSpares = useMemo(function () {
        ...
        if (selectedComponentId) {
            filtered = filtered.filter(function (spare) { return spare.componentId === selectedComponentId; });
        }
    ...
    var handleBulkUpdateChange = function (spareId, field, value) {
        ...
        setBulkUpdateData(function (prev) {
            var _a, _b;
            return (__assign(__assign({}, prev), (_a = {}, _a[spareId] = __assign(__assign({}, prev[spareId]), (_b = {}, _b[field] = numValue, _b)), _a)));
        });
    };
    // Save bulk updates
    ...
    var sparesWithStock = useMemo(function () {
        return filteredSpares.map(function (spare) { return (__assign(__assign({}, spare), { stock: getStockStatus(spare.rob, spare.min) })); });
    }, [filteredSpares]);
        ...
        return nodes.map(function (node) {
          ...
          <div className={"flex items-center px-3 py-2 cursor-pointer hover:bg-gray-50 border-b border-gray-100 ".concat(isSelected ? 'bg-[#52baf3] text-white' : '')} style={{ paddingLeft: "".concat(level * 20 + 12, "px") }} onClick={function () { return selectComponent(node.id); }}>
            <button className='mr-2 flex-shrink-0' onClick={function (e) {
              ...
              {hasChildren ? (isExpanded ? (<ChevronDown className={"h-4 w-4 ".concat(isSelected ? 'text-white' : 'text-gray-600')}/>) : (<ChevronRight className={"h-4 w-4 ".concat(isSelected ? 'text-white' : 'text-gray-600')}/>)) : (<ChevronRight className={"h-4 w-4 ".concat(isSelected ? 'text-white' : 'text-gray-400')}/>)}
            </button>
    ...
    return (<div className='h-full p-6 bg-[#fafafa]'>
      ...
      {activeTab === 'inventory' ? (<div className='flex gap-6 h-[calc(100vh-200px)]'>
          ...
          <div className='w-[70%]'>
            <div className='bg-white rounded-lg'>
              <AgGridTable rowData={sparesWithStock} columnDefs={columnDefs} onGridReady={onGridReady} context={gridContext} height='calc(100vh - 280px)' enableExport={true} enableSideBar={true} enableStatusBar={true} pagination={true} paginationPageSize={50} animateRows={true} suppressRowClickSelection={true} className='rounded-lg shadow-sm'/>
            </div>
          </div>
        </div>) : (
        // History View
          ...
          <div className='w-[70%]'>
            ...
            <div className='bg-white rounded-lg shadow-sm border'>
              ...
              <div className='divide-y divide-gray-200'>
                {historyData.map(function (entry) { return (<div key={entry.id} className='px-4 py-3'>
                    <div className='grid grid-cols-6 gap-4 text-sm items-center'>
                      <div className='text-gray-900'>{entry.date}</div>
                      <div className='text-gray-900'>{entry.partName}</div>
                      <div>
                        <span className={"px-2 py-1 rounded text-xs ".concat(entry.type === 'Consumed'
      ...
      {/* Add Spares Modal */}
      {isAddSpareModalOpen && (<div className='fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50'>
          <div className='bg-white rounded-lg shadow-lg w-[95%] max-w-7xl max-h-[90vh] overflow-auto'>
            ...
            <div className='flex justify-between items-center p-4 border-b'>
              ...
              <Button variant='ghost' size='sm' onClick={function () { return setIsAddSpareModalOpen(false); }} className='h-8 w-8 p-0 ml-[90px] mr-[90px]'>
                <X className='h-4 w-4'/>
              </Button>
            </div>
          ...
          <div className='bg-white rounded-lg shadow-lg w-[95%] max-w-7xl max-h-[90vh] overflow-auto'>
            ...
            <div className='p-6'>
              ...
              <div className='border border-t-0 rounded-b max-h-[400px] overflow-y-auto'>
                ...
                return (<div key={spare.id} className='grid grid-cols-8 gap-3 p-3 border-b bg-white items-center'>
                      ...
                      <div>
                        <Input type='number' min='0' className='text-sm h-8' placeholder='0' value={consumed || ''} onChange={function (e) {
                      ...
                      <div className={"text-sm font-medium ".concat(newRob < spare.min ? 'text-red-600' : 'text-gray-900')}>
                        {newRob}
                      </div>
                      <div className='flex justify-center'>
                        <Button variant='ghost' size='sm' className='h-8 w-8 p-0' onClick={function () {
                        var comment = prompt("Add comment for ".concat(spare.partName, ":"));
                        if (comment) {
                            console.log("Comment for ".concat(spare.partName, ": ").concat(comment));
                        }
                    }}>

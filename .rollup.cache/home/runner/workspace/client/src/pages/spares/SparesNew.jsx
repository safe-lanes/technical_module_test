...
var Spares = function () {
    ...
    var _o = useState({
        ...
        workOrder: '',
        remarks: '',
    }), consumeForm = _o[0], setConsumeForm = _o[1];
    var _p = useState({
        quantity: '',
        date: '',
        supplier: '',
        remarks: '',
    }), receiveForm = _p[0], setReceiveForm = _p[1];
    ...
    var isComponentMatch = function (spare, selectedId) {
        ...
        return spare.componentId.startsWith("".concat(selectedId, "."));
    };
    ...
    var filteredSpares = useMemo(function () {
        ...
        if (selectedComponentId) {
            filtered = filtered.filter(function (spare) {
                return isComponentMatch(spare, selectedComponentId);
            });
        }
          ...
          {activeTab === 'inventory' ? (<>
              ...
              <div className='overflow-y-auto h-[calc(100%-48px)]'>
                {isLoading ? (<div className='p-8 text-center text-gray-500'>
                    Loading...
                  </div>) : filteredSpares.length === 0 ? (<div className='p-8 text-center text-gray-500'>
                    No spares found. Try adjusting your filters.
                      ...
                      <div className='grid grid-cols-10 gap-4 text-sm items-center'>
                        ...
                        <div>
                          <span className={"px-2 py-1 rounded text-xs ".concat(spare.critical === 'Critical' ||
                    spare.critical === 'Yes'
              ...
              <div className='overflow-y-auto h-[calc(100%-48px)]'>
                {historyData.length === 0 ? (<div className='p-8 text-center text-gray-500'>
                    ...
                    : history.eventType === 'RECEIVE'
                        ...
                        <div className={"text-center font-semibold ".concat(history.qtyChange < 0
                    ? 'text-red-600'
                    : 'text-green-600')}>
                          {history.qtyChange > 0 ? '+' : ''}
                          {history.qtyChange}
      ...
      <Dialog open={isConsumeModalOpen} onOpenChange={setIsConsumeModalOpen}>
        <DialogContent>
          ...
          <div className='space-y-4'>
            <div>
              <Label>
                Part: {selectedSpare === null || selectedSpare === void 0 ? void 0 : selectedSpare.partCode} - {selectedSpare === null || selectedSpare === void 0 ? void 0 : selectedSpare.partName}
              </Label>
              <p className='text-sm text-gray-500'>
                Current ROB: {selectedSpare === null || selectedSpare === void 0 ? void 0 : selectedSpare.rob}
              </p>
            </div>
            <div>
              <Label htmlFor='consume-quantity'>Quantity *</Label>
              <Input id='consume-quantity' type='number' min='1' max={selectedSpare === null || selectedSpare === void 0 ? void 0 : selectedSpare.rob} value={consumeForm.quantity} onChange={function (e) {
            return setConsumeForm(__assign(__assign({}, consumeForm), { quantity: e.target.value }));
        }} required/>
            </div>
            <div>
              <Label htmlFor='consume-date'>Date *</Label>
              <Input id='consume-date' type='date' value={consumeForm.date} onChange={function (e) {
            return setConsumeForm(__assign(__assign({}, consumeForm), { date: e.target.value }));
        }} required/>
            </div>
            <div>
              <Label htmlFor='consume-wo'>Work Order</Label>
              <Input id='consume-wo' value={consumeForm.workOrder} onChange={function (e) {
            return setConsumeForm(__assign(__assign({}, consumeForm), { workOrder: e.target.value }));
        }} placeholder='Optional'/>
            </div>
            <div>
              <Label htmlFor='consume-remarks'>Remarks</Label>
              <Input id='consume-remarks' value={consumeForm.remarks} onChange={function (e) {
            return setConsumeForm(__assign(__assign({}, consumeForm), { remarks: e.target.value }));
        }} placeholder='Optional'/>
            </div>
          </div>
          <DialogFooter>
        ...
        <DialogContent>
          ...
          <div className='space-y-4'>
            ...
            <div>
              ...
              <Input id='receive-quantity' type='number' min='1' value={receiveForm.quantity} onChange={function (e) {
            return setReceiveForm(__assign(__assign({}, receiveForm), { quantity: e.target.value }));
        }} required/>
            </div>
            <div>
              <Label htmlFor='receive-date'>Date *</Label>
              <Input id='receive-date' type='date' value={receiveForm.date} onChange={function (e) {
            return setReceiveForm(__assign(__assign({}, receiveForm), { date: e.target.value }));
        }} required/>
            </div>
            <div>
              <Label htmlFor='receive-supplier'>Supplier/PO</Label>
              <Input id='receive-supplier' value={receiveForm.supplier} onChange={function (e) {
            return setReceiveForm(__assign(__assign({}, receiveForm), { supplier: e.target.value }));
        }} placeholder='Optional'/>
            </div>
            <div>
              <Label htmlFor='receive-remarks'>Remarks</Label>
              <Input id='receive-remarks' value={receiveForm.remarks} onChange={function (e) {
          ...
          <div className='space-y-4'>
            ...
            return (<tr key={spare.id} className={"border-t ".concat(hasError ? 'bg-red-50' : '')}>
                        ...
                        <td className='px-4 py-2'>
                          <Input type='number' min='0' max={spare.rob} value={((_d = bulkUpdateData[spare.id]) === null || _d === void 0 ? void 0 : _d.consumed) || ''} onChange={function (e) {
                    return handleBulkUpdateChange(spare.id, 'consumed', e.target.value);
                }} className={"w-20 mx-auto ".concat(hasInsufficientStock ? 'border-red-500' : '')}/>
                        </td>
                        <td className='px-4 py-2'>
                          <Input type='number' min='0' value={((_e = bulkUpdateData[spare.id]) === null || _e === void 0 ? void 0 : _e.received) || ''} onChange={function (e) {
                    return handleBulkUpdateChange(spare.id, 'received', e.target.value);
                }} className='w-20 mx-auto'/>
                        </td>
                        <td className='px-4 py-2 text-center'>
                          <div className={"text-sm font-medium ".concat(hasInsufficientStock ? 'text-red-600' : '')}>
                            {newROB}
                            {hasInsufficientStock && (<div className='text-xs text-red-600'>
                                Insufficient stock
                              </div>)}
                            {needsReceivedDate && (<div className='text-xs text-red-600'>
                                Date required
                              </div>)}
                          </div>
                        </td>
          ...
          <div className='space-y-4'>
            <div className='grid grid-cols-2 gap-4'>
              <div>
                ...
                <Input id='add-part-code' value={addSpareForm.partCode} onChange={function (e) {
            return setAddSpareForm(__assign(__assign({}, addSpareForm), { partCode: e.target.value }));
        }} placeholder='e.g., SP-ME-001' required/>
              </div>
              <div>
                <Label htmlFor='add-part-name'>Part Name *</Label>
                <Input id='add-part-name' value={addSpareForm.partName} onChange={function (e) {
            return setAddSpareForm(__assign(__assign({}, addSpareForm), { partName: e.target.value }));
        }} placeholder='e.g., Fuel Injector' required/>
              </div>
            </div>

            <div>
              <Label htmlFor='add-component'>Linked Component *</Label>
              <Select value={addSpareForm.componentId} onValueChange={function (value) {
            return setAddSpareForm(__assign(__assign({}, addSpareForm), { componentId: value }));
        }}>
            ...
            <div className='grid grid-cols-3 gap-4'>
              <div>
                ...
                <Select value={addSpareForm.critical} onValueChange={function (value) {
            return setAddSpareForm(__assign(__assign({}, addSpareForm), { critical: value }));
        }}>
              ...
              <div>
                ...
                <Input id='add-rob' type='number' min='0' value={addSpareForm.rob} onChange={function (e) {
            return setAddSpareForm(__assign(__assign({}, addSpareForm), { rob: e.target.value }));
        }} placeholder='0'/>
              </div>
              <div>
                <Label htmlFor='add-min'>Minimum Stock</Label>
                <Input id='add-min' type='number' min='0' value={addSpareForm.min} onChange={function (e) {
            return setAddSpareForm(__assign(__assign({}, addSpareForm), { min: e.target.value }));
        }} placeholder='0'/>
              </div>
            </div>
            ...
            <div>
              ...
              <Input id='add-location' value={addSpareForm.location} onChange={function (e) {
            return setAddSpareForm(__assign(__assign({}, addSpareForm), { location: e.target.value }));

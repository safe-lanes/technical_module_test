...
export var TechnicalModule = function () {
    ...
    var _a = getStateFromUrl(), subModule = _a.subModule, menuItem = _a.menuItem;
    var _b = useState(subModule), selectedSubModule = _b[0], setSelectedSubModule = _b[1];
    var _c = useState(menuItem), selectedMenuItem = _c[0], setSelectedMenuItem = _c[1];
    ...
    return (<div className='min-h-screen bg-gray-50'>
      {/* Top Menu Bar */}
      <TopMenuBar selectedSubModule={selectedSubModule} onSubModuleChange={handleSubModuleChange}/>

      <div className='flex'>
        {/* Side Menu Bar */}
        <SideMenuBar subModule={selectedSubModule} selectedItem={selectedMenuItem} onItemSelect={handleMenuItemSelect}/>

        {/* Main Content Area */}
        <div className='flex-1'>
          {selectedSubModule === 'pms' && selectedMenuItem === 'components' ? (<Components />) : selectedSubModule === 'pms' &&
            selectedMenuItem === 'work-orders' ? (<WorkOrders />) : selectedSubModule === 'pms' &&
            selectedMenuItem === 'running-hrs' ? (<RunningHours />) : selectedSubModule === 'pms' && selectedMenuItem === 'spares' ? (<Spares />) : selectedSubModule === 'pms' && selectedMenuItem === 'stores' ? (<Stores />) : selectedSubModule === 'pms' &&
            selectedMenuItem === 'modify-pms' ? (<ModifyPMS />) : selectedSubModule === 'pms' && selectedMenuItem === 'admin' ? (<AdminPanel />) : selectedSubModule === 'admin' && selectedMenuItem === 'alerts' ? (<Alerts />) : (<div className='p-6'>

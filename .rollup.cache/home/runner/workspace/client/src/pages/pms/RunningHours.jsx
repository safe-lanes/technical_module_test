...
var RunningHours = function () {
    ...
    var _d = useModifyMode(), isModifyMode = _d.isModifyMode, targetId = _d.targetId, fieldChanges = _d.fieldChanges;
    ...
    var _m = useState(null), utilizationCache = _m[0], setUtilizationCache = _m[1];
    ...
    var _o = useState(mockData), runningHoursData = _o[0], setRunningHoursData = _o[1];
    // Fetch utilization rates asynchronously (non-blocking)
    useEffect(function () {
        var fetchUtilizationRates = function () { return __awaiter(void 0, void 0, void 0, function () {
            var rates_1, error_1;
                ...
                switch (_a.label) {
                    case 0:
                        ...
                        if (utilizationCache && (Date.now() - utilizationCache.timestamp < 15 * 60 * 1000)) {
                            ...
                            setRunningHoursData(function (prev) { return prev.map(function (item) { return (__assign(__assign({}, item), { utilizationRate: utilizationCache.data[item.id] || null })); }); });
                            return [2 /*return*/];
                        }
                        _a.label = 1;
                    case 1:
                    ...
                    case 2:
                        ...
                        // Apply rates to data
                        setRunningHoursData(function (prev) { return prev.map(function (item) { return (__assign(__assign({}, item), { utilizationRate: rates_1[item.id] || null })); }); });
                        return [3 /*break*/, 4];
                    case 3:
                        error_1 = _a.sent();
    ...
    var updateRunningHours = useMutation({
        ...
        onSuccess: function (_, variables) {
            // Update local data
            setRunningHoursData(function (prev) { return prev.map(function (item) {
                return item.id === variables.componentId
                    ? __assign(__assign({}, item), { runningHours: "".concat(variables.audit.cumulativeRH.toLocaleString(), " hrs"), lastUpdated: variables.dateUpdatedLocal }) : item;
            }); });
            // Bust cache
            ...
            if (variables.updates) {
                setRunningHoursData(function (prev) {
                    ...
                    variables.updates.forEach(function (update) {
                        var index = updatedData.findIndex(function (item) { return item.id === update.componentId; });
                        if (index !== -1) {
                            updatedData[index] = __assign(__assign({}, updatedData[index]), { runningHours: "".concat(update.cumulativeRH.toLocaleString(), " hrs"), lastUpdated: update.dateUpdatedLocal });
                        }
                    });
                    return updatedData;
                });
    ...
    var handleSaveUpdate = function () {
        ...
        var dateLocal = selectedDate.toLocaleDateString('en-GB', {
            day: '2-digit',
            month: 'short',
            year: 'numeric'
    ...
    var handleBulkSave = function () {
        ...
        for (var _i = 0, runningHoursData_1 = runningHoursData; _i < runningHoursData_1.length; _i++) {
            ...
            // Format date in vessel local time
            var dateLocal = today.toLocaleDateString('en-GB', {
                day: '2-digit',
                month: 'short',
                year: 'numeric'
    ...
    return (<div className="space-y-6">
      ...
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
        ...
        <div className="divide-y divide-gray-200">
          {(function () {
            var filteredData = runningHoursData.filter(function (item) {
                return !searchTerm || item.component.toLowerCase().includes(searchTerm.toLowerCase());
            });
            if (filteredData.length === 0 && searchTerm) {
                return (<div className="px-4 py-8 text-center text-gray-500">
                  No results found. <button onClick={clearFilters} className="text-blue-600 underline">Reset</button>
                </div>);
            }
            return filteredData.map(function (item) { return (<div key={item.id} className="px-4 py-3 hover:bg-gray-50">
              <div className="grid grid-cols-7 gap-4 text-sm items-center">
                ...
                <div className="text-gray-700" title="Computed from last 30 days of RH entries">
                  {item.utilizationRate !== null ? "".concat(item.utilizationRate, " hrs/day") : "—"}
                </div>
                <div className="col-span-2">
                  <Button variant="outline" size="sm" className="h-8 w-8 p-0" onClick={function () { return openUpdateDialog(item); }}>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Input type="number" value={updateForm.newValue} onChange={function (e) { return handleUpdateFormChange('newValue', e.target.value); }} className="mt-1" placeholder={updateMode === "addDelta" ? "100" : "20000"}/>
              </div>
            </div>
            
            <div>
              <Label className="text-sm text-gray-600">Date Updated</Label>
              <Input type="date" value={updateForm.dateUpdated} onChange={function (e) { return handleUpdateFormChange('dateUpdated', e.target.value); }} className="mt-1" max={new Date().toISOString().split('T')[0]}/>
            </div>

            {/* Meter Replaced Checkbox */}
            <div className="flex items-center space-x-2">
              <Checkbox id="meterReplaced" checked={meterReplaced} onCheckedChange={function (checked) { return setMeterReplaced(checked); }}/>
            ...
            {meterReplaced && (<div className="grid grid-cols-2 gap-4 p-3 bg-gray-50 rounded">
                <div>
                  ...
                  <Input type="number" value={updateForm.oldMeterFinal} onChange={function (e) { return handleUpdateFormChange('oldMeterFinal', e.target.value); }} className="mt-1" placeholder="Final reading" required/>
                </div>
                <div>
                  <Label className="text-sm text-gray-600">New Meter Start *</Label>
                  <Input type="number" value={updateForm.newMeterStart} onChange={function (e) { return handleUpdateFormChange('newMeterStart', e.target.value); }} className="mt-1" placeholder="0"/>
                </div>
              </div>)}

            <div>
              <Label className="text-sm text-gray-600">Comments</Label>
              <Textarea value={updateForm.comments} onChange={function (e) { return handleUpdateFormChange('comments', e.target.value); }} className="mt-1 resize-none" rows={3} placeholder={meterReplaced ? "Reason for meter replacement" : "Comments"}/>
            </div>
          </div>
      ...
      <Dialog open={isBulkUpdateOpen} onOpenChange={setIsBulkUpdateOpen}>
        <DialogContent className="w-[90vw] max-w-none h-[90vh] flex flex-col">
          ...
          <div className="flex-1 overflow-auto">
            ...
            var updateData = bulkUpdateData[item.id] || { value: "", meterReplaced: false };
            return (<div key={item.id} className="px-4 py-3">
                      <div className="grid grid-cols-5 gap-4 text-sm items-center">
                        ...
                        <div className="space-y-1">
                          <Input type="number" value={updateData.value || ""} onChange={function (e) { return handleBulkUpdateChange(item.id, 'value', e.target.value); }} placeholder={bulkUpdateMode === "addDelta" ? "Enter delta" : "Enter new value"} className="w-full"/>
                          {bulkUpdateErrors[item.id] && (<div className="text-red-500 text-xs">
                              {bulkUpdateErrors[item.id]}
                            </div>)}
                        </div>
                        <div className="flex items-center justify-center">
                          <Checkbox checked={updateData.meterReplaced || false} onCheckedChange={function (checked) { return handleBulkUpdateChange(item.id, 'meterReplaced', checked); }}/>
                        </div>
                      ...
                      {updateData.meterReplaced && (<div className="grid grid-cols-2 gap-4 mt-2 pl-10 pr-4">
                          <div>
                            ...
                            <Input type="number" value={updateData.oldMeterFinal || ""} onChange={function (e) { return handleBulkUpdateChange(item.id, 'oldMeterFinal', e.target.value); }} placeholder="Final reading" className="h-8 text-sm"/>
                          </div>
                          <div>
                            <Label className="text-xs text-gray-600">New Meter Start</Label>
                            <Input type="number" value={updateData.newMeterStart || ""} onChange={function (e) { return handleBulkUpdateChange(item.id, 'newMeterStart', e.target.value); }} placeholder="0" className="h-8 text-sm"/>
                          </div>
                        </div>)}
                    </div>);
        })}

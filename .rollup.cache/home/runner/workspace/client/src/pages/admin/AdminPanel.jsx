          ...
          {selectedTemplate ? (<Card className="p-6">
              ...
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-semibold">
                  {templates.findIndex(function (t) { return t.id === selectedTemplate.id; }) + 1}. {selectedTemplate.name}
                </h2>
              ...
              <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center mb-6" onDrop={handleFileDrop} onDragOver={function (e) { return e.preventDefault(); }}>
                ...
                {selectedFile && (<p className="mt-4 text-sm font-medium">{selectedFile.name}</p>)}
              </div>
                    ...
                    <Table>
                      ...
                      <TableBody>
                        {dryRunResult.rows.slice(0, 20).map(function (row) { return (<TableRow key={row.row}>
                            ...
                            <TableCell>
                              <Badge variant={row.status === 'ok' ? 'default' :
                            row.status === 'warning' ? 'secondary' :
                                'destructive'} className={row.status === 'ok' ? 'bg-green-100 text-green-800' :
                      ...
                      {(_a = history === null || history === void 0 ? void 0 : history.items) === null || _a === void 0 ? void 0 : _a.map(function (item) { return (<TableRow key={item.id}>
                          ...
                          <TableCell>
                            <div className="flex gap-2">
                              <Button size="sm" variant="ghost" onClick={function () { return handleDownloadHistoryFile(item.id, 'file'); }}>

...
export var changeRequestService = {
  ...
  getChangeRequestsByCategory: function (category) {
    return changeRequests.filter(function (req) {
      return req.category === category;
    });
  },
  // Get change request by ID
  getChangeRequestById: function (id) {
    return changeRequests.find(function (req) {
      return req.id === id;
    });
  },
  ...
  approveChangeRequest: function (id, approvedBy, comments) {
    var requestIndex = changeRequests.findIndex(function (req) {
      return req.id === id;
    });
    if (requestIndex === -1) return false;
    var request = changeRequests[requestIndex];
    var approvedDate = new Date().toISOString().split('T')[0];
    changeRequests[requestIndex] = __assign(__assign({}, request), {
      status: 'Approved',

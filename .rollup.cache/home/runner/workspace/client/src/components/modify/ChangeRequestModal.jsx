      ...
      <DialogContent className="max-w-4xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
          {tiles.map(function (tile) {
            ...
            return (<Card key={tile.id} className={"cursor-pointer transition-all duration-200 ".concat(tile.color, " hover:shadow-md")} onClick={function () { return handleTileClick(tile.id); }}>
                <CardHeader className="pb-3">
                  <div className="flex items-center space-x-3">
                    <div className={"p-2 rounded-lg bg-white shadow-sm"}>
                      <IconComponent className={"h-6 w-6 ".concat(tile.iconColor)}/>
                    </div>
                    <div>

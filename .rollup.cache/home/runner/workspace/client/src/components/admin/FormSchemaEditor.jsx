...
import { Save, X, Eye, Code, Settings, Plus, Trash2,
// Copy,
// Move,
 } from 'lucide-react';
...
export default function FormSchemaEditor(_a) {
    ...
    var _d = useState(false), publishDialogOpen = _d[0], setPublishDialogOpen = _d[1];
    var _e = useState(''), changelog = _e[0], setChangelog = _e[1];
    ...
    var updateSchemaMutation = useMutation({
        mutationFn: function (schemaJson) {
            return fetch("/api/admin/forms/".concat(version.formId, "/versions/").concat(version.id, "/schema"), {
                method: 'PUT',
    ...
    var publishMutation = useMutation({
        mutationFn: function (changelog) {
            return fetch("/api/admin/forms/".concat(version.formId, "/versions/").concat(version.id, "/publish"), {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    var discardMutation = useMutation({
        mutationFn: function () {
            return fetch("/api/admin/forms/".concat(version.formId, "/versions/").concat(version.id, "/discard"), {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            }).then(function (res) { return res.json(); });
        },
            ...
            <TabsContent value='visual' className='flex-1 overflow-y-auto'>
              <div className='space-y-4 p-4'>
                ...
                color: 'white',
                          ...
                          <Input value={section.title} onChange={function (e) {
                return updateSection(sIndex, __assign(__assign({}, section), { title: e.target.value }));
            }} className='max-w-xs'/>
                        </div>
                        <div className='flex gap-2'>
                          <Select value={section.layout} onValueChange={function (value) {
                return updateSection(sIndex, __assign(__assign({}, section), { layout: value }));
            }}>
                    ...
                    <CardContent>
                      <div className='space-y-2'>
                        ...
                        {section.fields.map(function (field, fIndex) { return (<div key={field.key} className='flex gap-2 items-center p-2 border rounded'>
                            <Input value={field.key} onChange={function (e) {
                    return updateField(sIndex, fIndex, __assign(__assign({}, field), { key: e.target.value }));
                }} placeholder='Field Key' className='w-32'/>
                            <Input value={field.label} onChange={function (e) {
                    return updateField(sIndex, fIndex, __assign(__assign({}, field), { label: e.target.value }));
                }} placeholder='Label' className='flex-1'/>
                            <Select value={field.type} onValueChange={function (value) {
                    return updateField(sIndex, fIndex, __assign(__assign({}, field), { type: value }));
                }}>
                              <SelectTrigger className='w-32'>
                                <SelectValue />
                              </SelectTrigger>
                            ...
                            <label className='flex items-center gap-1'>
                              <input type='checkbox' checked={field.required} onChange={function (e) {
                    return updateField(sIndex, fIndex, __assign(__assign({}, field), { required: e.target.checked }));
                }}/>
                              Required
                            </label>
              ...
              <div className='p-4'>
                <div className='border rounded-lg p-6 bg-gray-50'>
                  ...
                  {schema.sections.map(function (section) { return (<div key={section.key} className='mb-6'>
                      ...
                      <div className={"grid gap-4 ".concat(section.layout === 'grid-1'
                ? 'grid-cols-1'

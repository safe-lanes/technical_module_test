...
import { EditIcon, EyeIcon, FilterIcon, SearchIcon, Trash2Icon, } from "lucide-react";
...
export var BaseSubmoduleTable = function (_a) {
    var title = _a.title, data = _a.data, columns = _a.columns, _b = _a.filters, filters = _b === void 0 ? [] : _b, onEdit = _a.onEdit, onView = _a.onView, onDelete = _a.onDelete, _c = _a.isLoading, isLoading = _c === void 0 ? false : _c, _d = _a.activeModule, activeModule = _d === void 0 ? "Appraisals" : _d;
    var _e = useState(true), showFilters = _e[0], setShowFilters = _e[1];
    var _f = useState({}), filterValues = _f[0], setFilterValues = _f[1];
    ...
    return (<div className="bg-transparent flex flex-row justify-center w-full">
        ...
        <header className="w-full h-[67px] bg-[#E8E8E8] border-b-2 border-[#5DADE2]">
          <div className="flex items-center h-full">
            ...
            <div className="flex items-center ml-4">
              <img className="w-14 h-10" alt="Logo" src="/figmaAssets/group-2.png"/>
            </div>
            ...
            <div className="absolute top-2.5 right-[38px]">
              <img className="w-[38px] h-[37px]" alt="User" src="/figmaAssets/group-3.png"/>
            </div>
          </div>
        </header>
        ...
        <main className="absolute top-[67px] left-[67px] w-[calc(100%-67px)] h-[calc(100%-67px)]">
          <div className="p-6">
            <div className="flex justify-between items-center mb-6">
              ...
              <Button variant="outline" className="h-10 border-[#e1e8ed] text-[#16569e] flex items-center gap-2 ml-[19px] mr-[19px]" onClick={function () { return setShowFilters(!showFilters); }}>
            ...
            {/* Filters */}
            {showFilters && filters.length > 0 && (<div className="flex justify-between items-center gap-2 mb-6 ml-4 mr-4">
                <div className="flex gap-2">
                  {filters.map(function (filter) {
                var _a;
                return (<div key={filter.key} className="relative">
                      {filter.type === 'search' ? (<div className="relative w-[180px]">
                          <Input className="h-8 pl-10 text-[#8798ad] text-xs" placeholder={filter.placeholder} value={filterValues[filter.key] || ''} onChange={function (e) { return handleFilterChange(filter.key, e.target.value); }}/>
                          <SearchIcon className="w-4 h-4 absolute left-3 top-2 text-[#8798ad]"/>
                        </div>) : (<Select value={filterValues[filter.key] || ''} onValueChange={function (value) { return handleFilterChange(filter.key, value); }}>
                          <SelectTrigger className="w-[150px] h-8 bg-white text-[#8a8a8a] text-xs">
                            <SelectValue placeholder={filter.placeholder}/>
                          </SelectTrigger>
                          <SelectContent>
                            {(_a = filter.options) === null || _a === void 0 ? void 0 : _a.map(function (option) { return (<SelectItem key={option.value} value={option.value}>
                                {option.label}
                              </SelectItem>); })}
                          </SelectContent>
                        </Select>)}
                ...
                <div className="flex gap-2">
                  ...
                  <Button variant="outline" className="h-8 w-20 text-[#8798ad] text-xs border-[#e1e8ed]" onClick={clearFilters}>
                    Clear
                  </Button>
                </div>
              </div>)}
              ...
              <CardContent className="p-4 bg-[#f7fafc]">
                <Table className="bg-white rounded-lg shadow-md overflow-hidden">
                  <TableHeader className="bg-[#52baf3]">
                    <TableRow>
                      {columns.map(function (column) { return (<TableHead key={column.key} className="text-white text-xs font-normal">
                          {column.header}
                        </TableHead>); })}
                  ...
                  <TableBody className="bg-white">
                    {filteredData.map(function (item, index) { return (<TableRow key={index} className="border-b border-gray-200 bg-white hover:bg-gray-50">
                        {columns.map(function (column) { return (<TableCell key={column.key} className="text-[#4f5863] text-[13px] font-normal py-3">
                            {column.render ? column.render(item) : item[column.key]}
                          </TableCell>); })}
                        <TableCell>
                          <div className="flex gap-2 justify-center">
                            {onView && (<Button variant="ghost" size="icon" className="h-6 w-6" onClick={function () { return onView(item); }}>
                                <EyeIcon className="h-[18px] w-[18px] text-gray-500"/>
                              </Button>)}
                            {onEdit && (<Button variant="ghost" size="icon" className="h-6 w-6" onClick={function () { return onEdit(item); }}>
                                <EditIcon className="h-[18px] w-[18px] text-gray-500"/>
                              </Button>)}
                            {onDelete && (<Button variant="ghost" size="icon" className="h-6 w-6" onClick={function () { return onDelete(item); }}>
            ...
            <div className="mt-4 text-xs font-normal font-['Mulish',Helvetica] text-black">
              {filteredData.length > 0 ? "1 to ".concat(filteredData.length, " of ").concat(filteredData.length) : "0 to 0 of 0"}
            </div>
          </div>
        </main>
      </div>
...
//# sourceMappingURL=BaseSubmoduleTable.jsx.map
      ...
      <div className='overflow-hidden bg-[url(/figmaAssets/vector.svg)] bg-[100%_100%] w-[1440px] h-[900px] relative'>
        ...
        <main className='absolute top-[67px] left-[67px] w-[calc(100%-67px)] h-[calc(100%-67px)]'>
          <div className='p-6'>
            ...
            {showFilters && filters.length > 0 && (<div className='flex justify-between items-center gap-2 mb-6 ml-4 mr-4'>
                ...
                return (<div key={filter.key} className='relative'>
                      {filter.type === 'search' ? (<div className='relative w-[180px]'>
                          <Input className='h-8 pl-10 text-[#8798ad] text-xs' placeholder={filter.placeholder} value={filterValues[filter.key] || ''} onChange={function (e) {
                            return handleFilterChange(filter.key, e.target.value);
                        }}/>
                          <SearchIcon className='w-4 h-4 absolute left-3 top-2 text-[#8798ad]'/>
                        </div>) : (<Select value={filterValues[filter.key] || ''} onValueChange={function (value) {
                            return handleFilterChange(filter.key, value);
                        }}>
            ...
            ? "1 to ".concat(filteredData.length, " of ").concat(filteredData.length)
            : '0 to 0 of 0'}
            </div>
          </div>
        </main>

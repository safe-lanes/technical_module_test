...
var SparesChangeRequestForm = function (_a) {
    ...
    var _c = useState([
        ...
        { partCode: "", partName: "", linkedComponent: "", qty: "", minQty: "", critical: "", location: "" }
    ]), spareItems = _c[0], setSpareItems = _c[1];
    ...
    var getLabelStyle = function () { return 'text-[#52baf3] text-sm font-medium'; };
    return (<div className="w-[95%] max-w-7xl max-h-[90vh] overflow-auto bg-white rounded-lg shadow-lg">
      ...
      <div className="flex justify-between items-center p-4 border-b border-[#52baf3]">
        ...
        <Button variant="ghost" size="sm" onClick={onClose} className="h-8 w-8 p-0 ml-[90px] mr-[90px] text-[#52baf3] hover:bg-[#52baf3] hover:text-white">
          <X className="h-4 w-4"/>
        </Button>
      </div>
      ...
      <div className="p-6">
        ...
        <div className="grid grid-cols-12 gap-3 bg-gray-50 p-3 rounded-t text-sm font-medium border border-[#52baf3]">
          ...
          <div className={"col-span-3 ".concat(getLabelStyle())}>Linked Component</div>
        ...
        <div className="border border-t-0 rounded-b border-[#52baf3]">
          {spareItems.map(function (item, index) { return (<div key={index} className="grid grid-cols-12 gap-3 p-3 border-b bg-white items-center">
              <div className="col-span-2">
                <Input value={item.partCode} onChange={function (e) { return handleInputChange(index, 'partCode', e.target.value); }} placeholder={index === 0 ? "SP-ME-001" : ""} className={getInputStyle(index, 'partCode')}/>
              </div>
              <div className="col-span-2">
                <Input value={item.partName} onChange={function (e) { return handleInputChange(index, 'partName', e.target.value); }} placeholder={index === 0 ? "Fuel Injector" : ""} className={getInputStyle(index, 'partName')}/>
              </div>
              <div className="col-span-3">
                <Select value={item.linkedComponent} onValueChange={function (value) { return handleInputChange(index, 'linkedComponent', value); }}>
              ...
              <Input value={item.qty} onChange={function (e) { return handleInputChange(index, 'qty', e.target.value); }} placeholder={index === 0 ? "1" : ""} className={getInputStyle(index, 'qty')}/>
              <Input value={item.minQty} onChange={function (e) { return handleInputChange(index, 'minQty', e.target.value); }} placeholder={index === 0 ? "1" : ""} className={getInputStyle(index, 'minQty')}/>
              <Select value={item.critical} onValueChange={function (value) { return handleInputChange(index, 'critical', value); }}>
              ...
              <div className="col-span-2 flex items-center gap-2">
                <Input value={item.location} onChange={function (e) { return handleInputChange(index, 'location', e.target.value); }} placeholder={index === 0 ? "Store Room A" : ""} className={"flex-1 ".concat(getInputStyle(index, 'location'))}/>

...
export function TargetPicker(_a) {
  ...
  var open = _a.open,
    ...
    onTargetSelect = _a.onTargetSelect;
  var _b = useState(''),
    searchQuery = _b[0],
    setSearchQuery = _b[1];
  ...
  var getComponentPath = function (component) {
    ...
    var buildPath = function (node, allComponents) {
      var parts = [
        ''.concat(node.componentCode || node.code, ' ').concat(node.name),
      ];
      if (node.parentId) {
        var parent_1 = allComponents.find(function (c) {
          return (c.id || c.componentId) === node.parentId;
        });
        if (parent_1) {
          return __spreadArray(
            __spreadArray([], buildPath(parent_1, allComponents), true),
            parts,
            true
          );
        }
    ...
    return ''
      .concat(component.componentCode || component.code, ' ')

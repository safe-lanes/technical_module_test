...
import { Plus, Trash2, X, ChevronRight, ChevronDown, Search, Save, Edit2, Link } from "lucide-react";
...
var generateMockComponentData = function (component) {
    ...
    var isAccommodation = component.code.startsWith("3");
    var baseData = {
        code: component.code,
        name: component.name,
...
export default function ComponentRegisterFormCR(_a) {
    ...
    var isChangeMode = useChangeMode().isChangeMode;
    ...
    var _h = useState(new Set()), editingWorkOrders = _h[0], setEditingWorkOrders = _h[1];
    var _j = useState({}), workOrderErrors = _j[0], setWorkOrderErrors = _j[1];
    // State for Spares
    var _k = useState(false), showSpareLinkPicker = _k[0], setShowSpareLinkPicker = _k[1];
    var _l = useState(new Set()), selectedNewSpares = _l[0], setSelectedNewSpares = _l[1];
    var _m = useState(""), spareSearchQuery = _m[0], setSpareSearchQuery = _m[1];
    // State for expanded sections
    var _o = useState(new Set(["A", "B", "C", "D", "E", "F", "G", "H"])), expandedSections = _o[0], setExpandedSections = _o[1];
    ...
    // Track field changes
    var trackFieldChange = useCallback(function (path, newValue, section) {
        if (!originalData || !componentData)
            return;
        var originalValue = path.split('.').reduce(function (obj, key) { return obj === null || obj === void 0 ? void 0 : obj[key]; }, originalData);
    ...
    var updateSectionChangeCount = useCallback(function () {
        var counts = {};
        changeTracking.forEach(function (change, path) {
        ...
        if (componentData === null || componentData === void 0 ? void 0 : componentData.workOrders) {
            var woChanges = componentData.workOrders.filter(function (wo) {
                return wo.isNew || wo.pendingDelete || wo.isEditing;
            }).length;
            if (woChanges > 0)
        ...
        if (componentData === null || componentData === void 0 ? void 0 : componentData.spares) {
            var spareChanges = componentData.spares.filter(function (spare) {
                return spare.isLinkedNew || spare.pendingUnlink || spare.isEditing;
            }).length;
            if (spareChanges > 0)
    ...
    var handleEditWorkOrder = function (woId) {
        ...
        setEditingWorkOrders(function (prev) { return new Set(prev).add(woId); });
        setComponentData(function (prev) { return (__assign(__assign({}, prev), { workOrders: prev.workOrders.map(function (wo) {
                return wo.id === woId ? __assign(__assign({}, wo), { isEditing: true, originalData: __assign({}, wo) }) : wo;
            }) })); });
    };
    var handleSaveWorkOrderEdit = function (woId) {
        ...
        setEditingWorkOrders(function (prev) {
    ...
    var handleCancelWorkOrderEdit = function (woId) {
        ...
        setComponentData(function (prev) { return (__assign(__assign({}, prev), { workOrders: prev.workOrders.map(function (wo) {
                if (wo.id === woId && wo.originalData) {
                    var originalData_1 = wo.originalData, isEditing = wo.isEditing, rest = __rest(wo, ["originalData", "isEditing"]);
    ...
    var handleAddWorkOrder = function () {
        ...
        setEditingWorkOrders(function (prev) { return new Set(prev).add(newWO.id); });
        updateSectionChangeCount();
    };
    ...
    var buildChangeRequestPayload = function () {
        ...
        var sectionAFields = ['maker', 'model', 'serialNo', 'location', 'critical',
        ...
        sectionAFields.forEach(function (field) {
            if (componentData[field] !== originalData[field]) {
                diff["A.".concat(field)] = {
        ...
        var sectionBFields = ['runningHours', 'dateUpdated', 'utilizationRate',
            ...
            if (wo.isNew && !wo.pendingDelete) {
                ...
                Object.keys(wo.originalData).forEach(function (key) {
                    if (wo[key] !== wo.originalData[key]) {
                        fields_1[key] = { from: wo.originalData[key], to: wo[key] };
                    }
                });
    ...
    var handleSubmit = function () {
        ...
        var invalidWOs = componentData === null || componentData === void 0 ? void 0 : componentData.workOrders.filter(function (wo) {
            return wo.isEditing && (!wo.jobTitle || !wo.frequencyType || !wo.frequencyValue || wo.frequencyValue <= 0);
        });
            ...
            {expandedSections.has('A') && (<div className="space-y-6">
                <div className="grid grid-cols-3 gap-6">
                  <div className="space-y-2">
                    ...
                    <Input value={componentData.maker} onChange={function (e) { return handleInputChange('maker', e.target.value, 'A'); }} className={getFieldClass('A.maker', "border-[#52baf3] border-2")}/>
                  </div>
                  <div className="space-y-2">
                    <Label className={getLabelClass('A.model')}>Model</Label>
                    <Input value={componentData.model} onChange={function (e) { return handleInputChange('model', e.target.value, 'A'); }} className={getFieldClass('A.model', "border-[#52baf3] border-2")}/>
                  </div>
                  <div className="space-y-2">
                    <Label className={getLabelClass('A.serialNo')}>Serial No</Label>
                    <Input value={componentData.serialNo} onChange={function (e) { return handleInputChange('serialNo', e.target.value, 'A'); }} className={getFieldClass('A.serialNo', "border-[#52baf3] border-2")}/>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-6">
                  <div className="space-y-2">
                    ...
                    <Input value={componentData.drawingNo} onChange={function (e) { return handleInputChange('drawingNo', e.target.value, 'A'); }} className={getFieldClass('A.drawingNo', "border-[#52baf3] border-2")}/>
                  </div>
                  <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={componentData.componentCategory} onValueChange={function (value) { return handleInputChange('componentCategory', value, 'A'); }}>
                ...
                <div className="grid grid-cols-3 gap-6">
                  <div className="space-y-2">
                    ...
                    <Input value={componentData.location} onChange={function (e) { return handleInputChange('location', e.target.value, 'A'); }} className={getFieldClass('A.location', "border-[#52baf3] border-2")}/>
                  </div>
                  <div className="space-y-2">
                    <Label className={getLabelClass('A.critical')}>Critical</Label>
                    <Select value={componentData.critical} onValueChange={function (value) { return handleInputChange('critical', value, 'A'); }}>
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={componentData.conditionBased} onValueChange={function (value) { return handleInputChange('conditionBased', value, 'A'); }}>
                ...
                <div className="grid grid-cols-3 gap-6">
                  <div className="space-y-2">
                    ...
                    <Input type="date" value={componentData.installation} onChange={function (e) { return handleInputChange('installation', e.target.value, 'A'); }} className={getFieldClass('A.installation', "border-[#52baf3] border-2")}/>
                  </div>
                  <div className="space-y-2">
                    <Label className={getLabelClass('A.commissionedDate')}>Commissioned Date</Label>
                    <Input type="date" value={componentData.commissionedDate} onChange={function (e) { return handleInputChange('commissionedDate', e.target.value, 'A'); }} className={getFieldClass('A.commissionedDate', "border-[#52baf3] border-2")}/>
                  </div>
                  <div className="space-y-2">
                    <Label className={getLabelClass('A.rating')}>Rating</Label>
                    <Input value={componentData.rating} onChange={function (e) { return handleInputChange('rating', e.target.value, 'A'); }} className={getFieldClass('A.rating', "border-[#52baf3] border-2")}/>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-6">
                  <div className="space-y-2">
                    ...
                    <Input value={componentData.noOfUnits} onChange={function (e) { return handleInputChange('noOfUnits', e.target.value, 'A'); }} className={getFieldClass('A.noOfUnits', "border-[#52baf3] border-2")}/>
                  </div>
                  <div className="space-y-2">
                    <Label className={getLabelClass('A.equipmentDepartment')}>Eqpt / System Department</Label>
                    <Input value={componentData.equipmentDepartment} onChange={function (e) { return handleInputChange('equipmentDepartment', e.target.value, 'A'); }} className={getFieldClass('A.equipmentDepartment', "border-[#52baf3] border-2")}/>
                  </div>
                  <div className="space-y-2">
                    <Label className={getLabelClass('A.parentComponent')}>Parent Component</Label>
                    <Input value={componentData.parentComponent} onChange={function (e) { return handleInputChange('parentComponent', e.target.value, 'A'); }} className={getFieldClass('A.parentComponent', "border-[#52baf3] border-2")} placeholder="Select parent component"/>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <Input value={componentData.dimensionsSize} onChange={function (e) { return handleInputChange('dimensionsSize', e.target.value, 'A'); }} className={getFieldClass('A.dimensionsSize', "border-[#52baf3] border-2")}/>
                  </div>
                  <div className="space-y-2">
                    <Label className={getLabelClass('A.notes')}>Notes</Label>
                    <Textarea value={componentData.notes} onChange={function (e) { return handleInputChange('notes', e.target.value, 'A'); }} className={getFieldClass('A.notes', "border-[#52baf3] border-2")} rows={3}/>
                  </div>
                </div>
              </div>)}
          </div>
            ...
            {expandedSections.has('B') && (<div className="space-y-6">
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <Input type="number" value={componentData.runningHours} onChange={function (e) { return handleInputChange('runningHours', e.target.value, 'B'); }} className={getFieldClass('B.runningHours', "border-[#52baf3] border-2")}/>
                  </div>
                  <div className="space-y-2">
                    <Label className={getLabelClass('B.dateUpdated')}>Date Updated</Label>
                    <Input type="date" value={componentData.dateUpdated} onChange={function (e) { return handleInputChange('dateUpdated', e.target.value, 'B'); }} className={getFieldClass('B.dateUpdated', "border-[#52baf3] border-2")}/>
                  </div>
                </div>
                      ...
                      <tbody>
                        {(_b = componentData.metrics) === null || _b === void 0 ? void 0 : _b.map(function (metric) { return (<tr key={metric.id} className={"\n                              ".concat(metric.pendingDelete ? 'strike-removed cr-changed-row' : '', "\n                              ").concat(metric.isNew ? 'cr-new-item' : '', "\n                              ").concat(metric.isEditing && !metric.isNew ? 'cr-modified-item' : '', "\n                            ")}>
                            <td className="px-4 py-2">
                              <Input value={metric.name} onChange={function (e) { return handleMetricFieldChange(metric.id, 'name', e.target.value); }} className={"h-8 ".concat(metric.isEditing || metric.isNew ? 'cr-changed' : '')} disabled={metric.pendingDelete}/>
                            </td>
                            <td className="px-4 py-2">
                              <Input type="number" value={metric.value} onChange={function (e) { return handleMetricFieldChange(metric.id, 'value', parseFloat(e.target.value)); }} className={"h-8 w-32 ".concat(metric.isEditing || metric.isNew ? 'cr-changed' : '')} disabled={metric.pendingDelete}/>
                            </td>
                            <td className="px-4 py-2">
                        ...
                        handleDeleteMetric(metric.id);
                              ...
                              {metric.pendingDelete && (<Button size="sm" variant="outline" onClick={function () { return handleRestoreMetric(metric.id); }} className="h-8">
                                  Restore
                                </Button>)}
                            </td>
                          </tr>); })}
                    ...
                    <tbody>
                      {componentData.workOrders.map(function (wo) { return (<tr key={wo.id} className={"\n                            ".concat(wo.pendingDelete ? 'strike-removed cr-changed-row' : '', "\n                            ").concat(wo.isNew ? 'cr-new-item' : '', "\n                            ").concat(wo.isEditing && !wo.isNew ? 'cr-modified-item' : '', "\n                          ")}>
                          {wo.isEditing && !wo.pendingDelete ? (<>
                              <td className="px-4 py-2">
                                <Input value={wo.jobTitle} onChange={function (e) { return handleWorkOrderFieldChange(wo.id, 'jobTitle', e.target.value); }} className={"h-8 ".concat(workOrderErrors[wo.id] && !wo.jobTitle ? 'border-red-500' : '')} placeholder="Job Title"/>
                              </td>
                              <td className="px-4 py-2">
                                <Input value={wo.assignedTo} onChange={function (e) { return handleWorkOrderFieldChange(wo.id, 'assignedTo', e.target.value); }} className="h-8" placeholder="Assigned To"/>
                              </td>
                              <td className="px-4 py-2">
                                <Select value={wo.frequencyType} onValueChange={function (value) { return handleWorkOrderFieldChange(wo.id, 'frequencyType', value); }}>
                              ...
                              <td className="px-4 py-2">
                                <Input type="number" value={wo.frequencyValue} onChange={function (e) { return handleWorkOrderFieldChange(wo.id, 'frequencyValue', parseInt(e.target.value)); }} className={"h-8 ".concat(workOrderErrors[wo.id] && wo.frequencyValue <= 0 ? 'border-red-500' : '')} placeholder="Value"/>
                              </td>
                              <td className="px-4 py-2">
                                <Input type="date" value={wo.initialNextDue} onChange={function (e) { return handleWorkOrderFieldChange(wo.id, 'initialNextDue', e.target.value); }} className="h-8"/>
                              </td>
                              <td className="px-4 py-2">
                                <Input value={wo.notes || ''} onChange={function (e) { return handleWorkOrderFieldChange(wo.id, 'notes', e.target.value); }} className="h-8" placeholder="Notes"/>
                              </td>
                              <td className="px-4 py-2">
                                <div className="flex gap-2">
                                  <Button size="sm" onClick={function () { return handleSaveWorkOrderEdit(wo.id); }} className="bg-green-600 hover:bg-green-700 text-white h-8">
                                    <Save className="h-4 w-4"/>
                                  </Button>
                                  <Button size="sm" variant="outline" onClick={function () { return handleCancelWorkOrderEdit(wo.id); }} className="h-8">
                                    <X className="h-4 w-4"/>
                                  </Button>
                                </div>
                              </td>
                  ...
                  <table className="w-full">
                    ...
                    <tbody>
                      {componentData.spares.map(function (spare) { return (<tr key={spare.id} className={"\n                            ".concat(spare.pendingUnlink ? 'strike-removed cr-changed-row' : '', "\n                            ").concat(spare.isLinkedNew ? 'cr-new-item' : '', "\n                            ").concat(spare.isEditing && !spare.isLinkedNew ? 'cr-modified-item' : '', "\n                          ")}>
                          ...
                          <td className="px-4 py-2">
                            <Input type="number" value={spare.min} onChange={function (e) { return handleEditSpare(spare.id, 'min', parseInt(e.target.value)); }} className={"h-8 w-20 ".concat(spare.isEditing ? 'cr-changed' : '')} disabled={spare.pendingUnlink}/>
                          </td>
                          <td className="px-4 py-2">
                            <Select value={spare.critical} onValueChange={function (value) { return handleEditSpare(spare.id, 'critical', value); }} disabled={spare.pendingUnlink}>
                          ...
                          <td className="px-4 py-2">
                            <Input value={spare.location} onChange={function (e) { return handleEditSpare(spare.id, 'location', e.target.value); }} className={"h-8 ".concat(spare.isEditing ? 'cr-changed' : '')} disabled={spare.pendingUnlink}/>
                          </td>
                          <td className="px-4 py-2">

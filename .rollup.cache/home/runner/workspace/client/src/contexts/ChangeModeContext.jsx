...
export var ChangeModeProvider = function (_a) {
    ...
    var _b = useState(false), isChangeMode = _b[0], setIsChangeMode = _b[1];
    var _c = useState(), changeRequestTitle = _c[0], setChangeRequestTitle = _c[1];
    var _d = useState(), changeRequestCategory = _d[0], setChangeRequestCategory = _d[1];
    var _e = useState(null), originalSnapshot = _e[0], setOriginalSnapshot = _e[1];
    var _f = useState([]), diffs = _f[0], setDiffs = _f[1];
    ...
    var collectDiff = function (path, oldVal, newVal) {
        ...
        if (JSON.stringify(oldVal) === JSON.stringify(newVal)) {
            ...
            setDiffs(function (prev) { return prev.filter(function (d) { return d.path !== path; }); });
            return;
        }
        setDiffs(function (prev) {
            var existing = prev.findIndex(function (d) { return d.path === path; });
            if (existing >= 0) {
                var updated = __spreadArray([], prev, true);
                updated[existing] = { path: path, oldVal: oldVal, newVal: newVal };
                return updated;
            }
            return __spreadArray(__spreadArray([], prev, true), [{ path: path, oldVal: oldVal, newVal: newVal }], false);
        });
    };
    var getDiffs = function () { return diffs; };
    var reset = function () {
        setDiffs([]);
    ...
    return (<ChangeModeContext.Provider value={{
...
export var useChangeModeField = function (fieldPath, originalValue) {
    var _a = useChangeMode(), isChangeMode = _a.isChangeMode, collectDiff = _a.collectDiff;
    var _b = useState(false), hasChanged = _b[0], setHasChanged = _b[1];

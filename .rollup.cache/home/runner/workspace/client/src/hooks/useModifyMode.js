...
export function useModifyMode() {
  ...
  var enableModifyMode = useCallback(
    function (targetType, targetId) {
      ...
      setLocation(
        ''.concat(location.split('?')[0], '?').concat(newParams.toString())
      );
    },
    [location, search, setLocation]
  );
  // Disable modify mode
  var disableModifyMode = useCallback(
    function () {
      var newParams = new URLSearchParams(search);
      ...
      setLocation(
        ''
          .concat(location.split('?')[0])
          .concat(newSearch ? '?'.concat(newSearch) : '')
      );
      // Clear field changes
      setState(function (prev) {
        return __assign(__assign({}, prev), {
  ...
  var setOriginalSnapshot = useCallback(function (snapshot) {
    setState(function (prev) {
      return __assign(__assign({}, prev), { originalSnapshot: snapshot });
    });
  }, []);
  // Track field changes
  var trackFieldChange = useCallback(function (fieldName, newValue, oldValue) {
    setState(function (prev) {
      ...
      if (newValue === originalValue) {
        var _b = prev.fieldChanges,
          _c = fieldName,
          removed = _b[_c],
          remainingChanges = __rest(_b, [
      ...
      return __assign(__assign({}, prev), {
        fieldChanges: __assign(
          __assign({}, prev.fieldChanges),
          ((_a = {}),
          (_a[fieldName] = {
            fieldName: fieldName,
  ...
  // Get change summary
  var getChangeSummary = useCallback(
    function () {
  ...
  var generateChangeRequestPayload = useCallback(
    function () {
      var changes = Object.values(state.fieldChanges);
      ...
      return {
        ...
        proposedChanges: changes.map(function (change) {
          return {

...
export var generateFormEditor = function (formName) {
  ...
  return '\nimport React, { useState } from "react";\nimport { useForm } from "react-hook-form";\nimport { zodResolver } from "@hookform/resolvers/zod";\nimport { z } from "zod";\nimport { Button } from "@/components/ui/button";\nimport { Card, CardContent } from "@/components/ui/card";\nimport { Input } from "@/components/ui/input";\nimport { Label } from "@/components/ui/label";\nimport { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";\nimport { Textarea } from "@/components/ui/textarea";\nimport { Checkbox } from "@/components/ui/checkbox";\nimport { ArrowLeft, Save, Settings } from "lucide-react";\nimport { Form } from "@shared/schema";\nimport { sailDesignSystem } from "@/config/sailDesignSystem";\n\nconst '
    .concat(formName.replace(/\s+/g, ''), 'Schema = ')
    .concat(JSON.stringify(template.schema, null, 2), ';\n\ntype ')
    .concat(formName.replace(/\s+/g, ''), 'Data = z.infer<typeof ')
    .concat(formName.replace(/\s+/g, ''), 'Schema>;\n\ninterface ')
    .concat(
      formName.replace(/\s+/g, ''),
      'EditorProps {\n  form: Form;\n  rankGroupName?: string;\n  onClose: () => void;\n  onSave: (data: any) => void;\n}\n\nexport const '
    )
    .concat(formName.replace(/\s+/g, ''), 'Editor: React.FC<')
    .concat(
      formName.replace(/\s+/g, ''),
      'EditorProps> = ({ \n  form, \n  rankGroupName, \n  onClose, \n  onSave \n}) => {\n  const [activeSection, setActiveSection] = useState("A");\n  const [isConfigMode, setIsConfigMode] = useState(false);\n\n  const formMethods = useForm<'
    )
    .concat(formName.replace(/\s+/g, ''), 'Data>({\n    resolver: zodResolver(')
    .concat(
      formName.replace(/\s+/g, ''),

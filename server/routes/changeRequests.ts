...
export default function createChangeRequestsRouter(storage: IStorage) {
  ...
  router.get('/', async (req, res) => {
    try {
      ...
      let requests = await storage.getChangeRequests({
  ...
  router.patch('/:id/status', async (req, res) => {
    try {
      ...
      if (
        !['draft', 'submitted', 'returned', 'approved', 'rejected'].includes(
          status
        )
      ) {
  ...
  router.post('/:id/comments', async (req, res) => {
    try {
      ...
      const newComment =
        await storage.createChangeRequestComment(validatedData);
  ...
  router.get('/:id/attachments', async (req, res) => {
    try {
      const changeRequestId = parseInt(req.params.id);
      const attachments =
        await storage.getChangeRequestAttachments(changeRequestId);
  ...
  router.post('/:id/attachments', async (req, res) => {
    try {
      ...
      const newAttachment =
        await storage.createChangeRequestAttachment(validatedData);
